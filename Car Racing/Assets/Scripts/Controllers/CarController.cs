...
public class CarController : MonoBehaviour
{
    ...
    public int current;
    private void calaculateDistanceofWaypoints()
    {
        Vector3 position = gameObject.transform.position;
        float distancesssss = Mathf.Infinity;

        for (int i = 0; i < nodes.Count; i++)
        {
            Vector3 difference = nodes[i].position - position;
            float currentdistance = difference.magnitude;
            if (currentdistance < distancesssss)
            {
                if ((i + distance) >= nodes.Count)
                {
                    currentWaypoint = nodes[1];
                    distancesssss = currentdistance;
                }
                else
                {
                    currentWaypoint = nodes[i + distance];
                    distancesssss = currentdistance;
                }
            }
            current = i;
